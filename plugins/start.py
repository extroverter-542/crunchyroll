...
async def start_command(client: Client, message):
    ...
    if len(text)>7:
                ...
                InlineKeyboardButton(
                    "Jᴏɪɴ Cʜᴀɴɴᴇʟ 1", url=client.link_one),
                        InlineKeyboardButton(
                          ...
                          ],[ InlineKeyboardButton("Jᴏɪɴ MAɪɴ Cʜᴀɴɴᴇʟ", url="https://t.me/+LbyEow2rfmM1OWFl")]
            ]
            try:
                btn.append(
                      [
        ...
        if client.link_two is not None and message.from_user.id not in ADMINS and not await is_requested_two(message):
            ...
            await client.send_message(
                chat_id=message.from_user.id,
                text="Kᴏɴɴɪᴄʜɪᴡᴀ Wᴇᴇʙ 👋,\n\nᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟs ᴀɴᴅ ᴛʜᴇɴ ᴄʟɪᴄᴋ ᴏɴ ♻️ Try Again ♻️ ʙᴜᴛᴛᴏɴ ᴛᴏ ɢᴇᴛ ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛᴇᴅ ꜰɪʟᴇ.\n\n𝐃ᴇᴠᴇʟᴏᴘᴇᴅ 𝐁ʏ : @Straw_Hat_Bots",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=btn),
                parse_mode=ParseMode.MARKDOWN
            )
            return
